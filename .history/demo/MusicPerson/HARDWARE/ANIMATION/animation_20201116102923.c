...
PRESTA:
    ...
    if (netpro & 0X02)                                                     //UDP
    {
        ...
        if (atk_8266_ip_set("WIFI-AP 远端UDP IP设置", (u8 *)ATK_ESP8266_WORKMODE_TBL[netpro], (u8 *)portnum, ipbuf))
            goto PRESTA;                                                           //IP输入
    ...
    else //TCP
    {
        if (netpro & 0X01) //TCP Client    透传模式测试
        {
            ...
            Show_Str_Mid(0, 30, "ATK-ESP WIFI-AP 测试", 16, 240);
            Show_Str(30, 50, 200, 16, "正在配置ATK-ESP模块,请稍等...", 12, 0);
            if (atk_8266_ip_set("WIFI-AP 远端IP设置", (u8 *)ATK_ESP8266_WORKMODE_TBL[netpro], (u8 *)portnum, ipbuf))
                goto PRESTA;                                                           //IP输入
    ...
    while (1)
    {
        ...
        else if (key == KEY0_PRES) //KEY0 发送数据
        {
            ...
            else //TCP Server
            {
                if (send == 1)
                    send = 0;
                Show_Str(30 + 54, 100, 200, 12, "正在发送,按下key1取消", 12, 0);
                ...
                while (!send)
                {
                    ...
                    for (i = 0; i < screenHeight; i++)
                    {
                        for (j = 0; j < screenWidth; j++)
                        {
                            //
                            //	if(j%2==0){
                            //		colorHL[i*480+j]=color>>8;
                            //		}

                            //		else{
                            //			colorHL[i*480+j]=color&0x00ff;
                            //		}
                            ...
                            colorHL[j] = color >> 8;
                            colorHL[j + 480] = color & 0x00ff;
                        }
                        atk_8266_send_cmd("AT+CIPSEND=0,960", "", 1); //发送指定长度的数据
                        atk_8266_send_data(colorHL, "", 1);           //发送指定长度的数据
                    }
                    ...
                    {
                        send = 1;
                        Show_Str(30 + 54, 100, 200, 12, "已取消", 12, 0);
                    }
                }
                timex = 100;
            }
